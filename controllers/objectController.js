...
const getObjByCode = async (req, res) => {
  ...
  try {
    ...
    const query = `SELECT * FROM vw_object_items WHERE code = @code order by id`;
...
const getObjectsByYearAndSubCode = async (req, res) => {
  ...
  try {
    const pool = await poolPromise;
    const query = `SELECT * FROM vw_object_items WHERE year = @year AND sub_code = @sub_code order by id`;
    const result = await pool
      .request()
      .input("year", sql.Int, year)
